...
const supabaseAdmin = createClient(supabaseUrl, supabaseServiceRoleKey || supabaseKey);
...
app.get('/api/products', async (req, res) => {
  ...
  try {
    ...
    if (category_id) {
      query = query.eq('category_id', category_id).order('created_at', { ascending: false });
    } else {
...
app.post('/api/products', async (req, res) => {
  try {
    const { title, details, price, category, location, imageData, imageName, uploader_id } = req.body;

    if (!title || !price || !category || !location) {
      return res.status(400).json({ error: 'Title, price, category, and location are required.' });
    }
    ...
    if (imageData && imageName) {
      try {
        ...
        if (uploadRes.error && uploadRes.error.message && uploadRes.error.message.toLowerCase().includes('bucket not found') && supabaseServiceRoleKey) {
          try {
            console.log('Creating missing storage bucket: products');
            const createRes = await supabaseAdmin.storage.createBucket('products', { public: true });
            if (createRes.error) {
        ...
        if (uploadRes.error) {
          ...
          const publicRes = storageClient.storage.from('products').getPublicUrl(fileName);
    ...
    const categoryResult = await supabase.from('categories').select('id').eq('name', category).maybeSingle();
    ...
    if (!categoryId) {
      const createCategoryResult = await supabase.from('categories').insert({ name: category }).select('id').single();
      if (createCategoryResult.error) {
        console.error('Category creation error:', createCategoryResult.error);
        return res.status(500).json({ error: createCategoryResult.error.message });
      }
